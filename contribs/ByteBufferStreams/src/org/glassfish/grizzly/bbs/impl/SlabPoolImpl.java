...
public final class SlabPoolImpl implements AllocatorFactory.SlabPool {
    ...
    private final Allocator.BufferType _bufferType ;
    ...
    public SlabPoolImpl( final int maxAllocationSize, final long minSize, 
        final long maxSize, final Allocator.BufferType bufferType ) {
    ...
    private long totalSpaceInUse() {
        ...
        result = result * (_emptySlabs.size() + _partialSlabs.size() 
            + _fullSlabs.size()) ;
        return result ;
    }
    ...
    public synchronized long freeSpace() {
        checkClosed() ;
        return computeAvailableSize( _emptySlabs ) 
            + computeAvailableSize( _partialSlabs ) ;
    }
    ...
    public synchronized long unavailableSpace() {
        checkClosed() ;
        return computeDisposedSize( _partialSlabs ) 
            + computeDisposedSize( _fullSlabs ) ;
    }

    public synchronized long allocatedSpace() {
        checkClosed() ;
        return computeAllocatedSize( _partialSlabs ) 
            + computeAllocatedSize( _fullSlabs ) ;
    }
