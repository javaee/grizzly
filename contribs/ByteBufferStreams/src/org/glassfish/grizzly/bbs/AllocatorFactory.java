...
import org.glassfish.grizzly.bbs.impl.SlabPoolImpl;
import org.glassfish.grizzly.bbs.impl.AllocatorPoolImpl;
import org.glassfish.grizzly.bbs.impl.AllocatorImpl;
...
public class AllocatorFactory {
     ...
     */
    public static Allocator makeAllocator( final int headerSize, 
        final int allocatorSize, final Allocator.BufferType bufferType ) {
        if (headerSize < MIN_HEADER_SIZE) {
            throw new IllegalArgumentException( "Request size " + headerSize 
                + " for headerSize is less than the minimum size " 
                + MIN_HEADER_SIZE ) ;
        }
        return new AllocatorImpl( headerSize, allocatorSize, bufferType ) ;
    }
     ...
     */
    public static Allocator makePoolAllocator( final int headerSize, 
        final SlabPool pool ) {
     ...
     */
    public SlabPool makeSlabPool( final int maxAllocationSize,  
        final long minSize, final long maxSize, 
        final Allocator.BufferType bufferType ) {
