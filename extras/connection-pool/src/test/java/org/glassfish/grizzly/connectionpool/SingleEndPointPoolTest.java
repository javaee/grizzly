...
public class SingleEndPointPoolTest {
    ...
    public void testPollTimeout() throws Exception {
        ...
        Connection c1 = pool.take(0, TimeUnit.MILLISECONDS);
        assertNull(c1);
        assertEquals(0, pool.size());
        
        c1 = pool.take(-1, TimeUnit.MILLISECONDS);
        assertNotNull(c1);
        assertEquals(1, pool.size());
        
        Connection c2 = pool.take(-1, TimeUnit.MILLISECONDS);
        assertNotNull(c2);
        assertEquals(2, pool.size());
        
        Connection c3 = pool.take(2, TimeUnit.SECONDS);
        assertNull(c3);
        assertEquals(2, pool.size());
        
        pool.release(c2);

        c3 = pool.take(2, TimeUnit.SECONDS);
        assertNotNull(c3);
        assertEquals(2, pool.size());
    }
