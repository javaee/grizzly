...
public class JkMX extends JkHandler {
    ...
    public void loadAdapter() throws IOException {
        ...
        if ((httpport != -1) && classExists("mx4j.adaptor.http.HttpAdaptor")) {
            try {
                ...
                LoggerUtils.getLogger().info("Started MX4J console on host " + httphost + " at port " + httpport);

                httpAdapterLoaded = true;

            } catch (Throwable t) {
                httpServerName = null;
                LoggerUtils.getLogger().log(Level.SEVERE, "Can't load the MX4J http adapter ", t);
            }
        }
        ...
        if ((jrmpport != -1) && classExists("mx4j.tools.naming.NamingService")) {
            try {
                ...
                LoggerUtils.getLogger().info("Creating " + jrmpServerName);
                ...
                LoggerUtils.getLogger().info("Creating " + adaptor + " on host " + jrmphost + " at port " + jrmpport);

                jrmpAdapterLoaded = true;

            } catch (Exception ex) {
                jrmpServerName = null;
                LoggerUtils.getLogger().log(Level.SEVERE, "MX4j RMI adapter not loaded: " + ex.toString());
            }
        }
        ...
        if ((httpport != -1) && (!httpAdapterLoaded) && classExists("com.sun.jdmk.comm.HtmlAdaptorServer")) {
            try {
                ...
                LoggerUtils.getLogger().info("Registering the JMX_RI html adapter " + httpServerName + " at port " + httpport);
                ...
                httpServerName = null;
                LoggerUtils.getLogger().log(Level.SEVERE, "Can't load the JMX_RI http adapter " + t.toString());
            }
        }

        if ((!httpAdapterLoaded) && (!jrmpAdapterLoaded)) {
            LoggerUtils.getLogger().log(Level.WARNING, "No adaptors were loaded but mx.enabled was defined.");
        }
    ...
    public void destroy() {
        try {
            ...
            LoggerUtils.getLogger().info("Stoping JMX ");
            ...
            LoggerUtils.getLogger().log(Level.SEVERE, "Destroy error" + t);
        }
    }
        ...
        try {
            ...
            if (log4jEnabled) {
                try {
                    ...
                    LoggerUtils.getLogger().info("Registering the JMX hierarchy for Log4J ");
                } catch (Throwable t) {
                }
            }
        } catch (Throwable t) {
            LoggerUtils.getLogger().log(Level.SEVERE, "Init error", t);
        }
    }
