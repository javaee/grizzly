...
public final class CompressedHeadersBuilder {
    ...
    /**
     * Utility method, which helps to create Spdy {@link Deflater} with predefined
    ...
    public final CompressedHeadersBuilder header(Header header, String value) {
        headers.put(header.toString().toLowerCase(Locale.US), value);
        return this;
    }
    
    public Buffer build(final Deflater deflater) throws IOException {
        final SpdyDeflaterOutputStream deflaterOutput =
                new SpdyDeflaterOutputStream(MemoryManager.DEFAULT_MEMORY_MANAGER,
                deflater);
        
        final DataOutputStream dataOutput = new DataOutputStream(deflaterOutput);
        
        try {
            dataOutput.writeInt(headers.size());
            
            for (Map.Entry<String, String> entry : headers.entrySet()) {
                final byte[] name = entry.getKey().getBytes(Charsets.ASCII_CHARSET.name());
                final byte[] value = entry.getValue().getBytes(Charsets.ASCII_CHARSET.name());
