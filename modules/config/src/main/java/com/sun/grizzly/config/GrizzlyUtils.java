...
public class GrizzlyUtils {
    /**
     * Reads bytes to the <code>WorkerThread</code> associated <code>ByteBuffer</code>s. Could be used to read both raw
     * and secured data.
     *
     ...
     * @throws IOException
     */
    public static int readToWorkerThreadBuffers(final SelectionKey key, final int timeout) throws IOException {
        ...
        if (sslEngine == null) {
            ...
            if (securedBuffer.position() > 0) {
                ...
                final int producedBytes = byteBuffer.position() - initialPosition;
                if (producedBytes > 0) {
