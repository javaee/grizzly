 ...
 */
public class UDPAsyncQueueReaderTest extends TestCase {
    public static final int PORT = 17516;
            ...
            for(int i=0; i<CLIENTS_COUNT; i++) {
                ...
                try {
                    ...
                    for(int j=0; j<PACKETS_COUNT; j++) {
                        ...
                        Future future = udpConnector.readFromAsyncQueue(readBB,
                        ...
                        if (!val1.equals(val2)) {
                            AsyncQueueReader reader = udpConnector.getSelectorHandler().getAsyncQueueReader();
                            AsyncQueueWriter writer = udpConnector.getSelectorHandler().getAsyncQueueWriter();
                            SelectionKey clientKey = udpConnector.getUnderlyingChannel().keyFor(udpConnector.getSelectorHandler().getSelector());
                            ...
                            AsyncQueue.AsyncQueueEntry readerQueueEntry = reader.getAsyncQueue(clientKey);
                            AsyncQueue.AsyncQueueEntry writerQueueEntry = writer.getAsyncQueue(clientKey);
                            Controller.logger().log(Level.INFO, "AsyncReadQueue total: " + readerQueueEntry.totalElementsCount +
                                    " processed: " + readerQueueEntry.processedElementsCount +
                                    " queued: " + readerQueueEntry.queuedElementsCount +
                                    " processedData(bytes): " + readerQueueEntry.processedDataSize +
                                    " hasElemeents?: " + reader.isReady(clientKey));
                            Controller.logger().log(Level.INFO, "AsyncWriteQueue total: " + writerQueueEntry.totalElementsCount +
                                    " processed: " + writerQueueEntry.processedElementsCount +
                                    " queued: " + writerQueueEntry.queuedElementsCount +
