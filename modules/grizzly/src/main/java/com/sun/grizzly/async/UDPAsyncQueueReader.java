...
public class UDPAsyncQueueReader extends AbstractAsyncQueueReader {
    ...
    protected OperationResult doRead(ReadableByteChannel channel,
        ...
        if (readPostProcessor != null) {
            ...
            doRead(channel, inputByteBuffer, dstResult);
            ...
            return doRead(channel, byteBuffer, dstResult);
        }
    }
    
    private OperationResult doRead(ReadableByteChannel channel,
            ByteBuffer byteBuffer, OperationResult dstResult)
            throws IOException {

        int pos = byteBuffer.position();
        SocketAddress address = ((DatagramChannel) channel).receive(byteBuffer);

        dstResult.address = address;
        dstResult.bytesProcessed = byteBuffer.position() - pos;
        return dstResult;
    }
}
