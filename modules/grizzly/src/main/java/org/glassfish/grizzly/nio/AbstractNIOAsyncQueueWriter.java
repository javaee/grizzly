...
public abstract class AbstractNIOAsyncQueueWriter
    ...
    public GrizzlyFuture<WriteResult<Buffer, SocketAddress>> write(
        ...
        final AsyncWriteQueueRecord queueRecord = createRecord(
                ...
                dstAddress, pushBackHandler, !buffer.hasRemaining());
        ...
        try {
            if (reentrants.incAndGet() >= maxWriteReentrants) {
                ...
                queueRecord.setMessage(
                        cloneRecordIfNeeded(connection, cloner, buffer));
    ...
    private static Buffer cloneRecordIfNeeded(
