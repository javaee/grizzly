...
public class DefaultConditionListener<E> extends ConditionListener<E, Object> {
    ...
    public DefaultConditionListener(Condition<E> condition, Object notificationObject) {
        super(condition, notificationObject);
    }

    public static final void notifyListenerObject(Object notificationObject) {
        if (notificationObject instanceof CountDownLatch) {
            ((CountDownLatch) notificationObject).countDown();
        } else if (notificationObject instanceof Callable) {
            try {
                ((Callable) notificationObject).call();
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        } else if (notificationObject instanceof Runnable) {
            ((Runnable) notificationObject).run();
        } else {
