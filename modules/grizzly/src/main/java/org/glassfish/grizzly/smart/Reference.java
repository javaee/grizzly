...
public class Reference<E> {
    ...
    public Reference(Class root, String ref) {
        try {
            buildPath(root, ref, path);
        } catch (Exception e) {
            throw new IllegalStateException(e);
        }
    }

    public E get(Object rootInstance) throws IllegalArgumentException,
            IllegalAccessException,
            InvocationTargetException {
        Object currentValue = rootInstance;
        
        for(AccessibleObject accessor : path) {
            if (accessor instanceof Field) {
                currentValue = ((Field) accessor).get(currentValue);
            } else {
                currentValue = ((Method) accessor).invoke(rootInstance);
            }
        }
