...
/**
 * Simple echo filter based on NIO.2
 *
 * @author Jeanfrancois Arcand
 */
public class AIOEchoFilter implements ProtocolFilter {
    public boolean execute(Context c) throws IOException {
        final WorkerThread workerThread = ((WorkerThread)Thread.currentThread());
        AIOContext ctx = (AIOContext)c;   
        ByteBuffer buffer = ctx.getByteBuffer();
        buffer.flip();
                
        if (buffer.hasRemaining()) {
            AsynchronousSocketChannel channel = ctx.getChannel();
            try {
                AIOOutputWriter.flushChannel(channel, buffer);
