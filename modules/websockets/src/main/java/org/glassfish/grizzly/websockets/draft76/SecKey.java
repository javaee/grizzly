...
public class SecKey {
    ...
    public static SecKey validateSecKey(String key) {
        if (key.length() > 256) {
            throw new HandshakeException("Key too long");
        }
        ...
        for (int i = 0; i < key.length(); i++) {
            ...
            if (c >= '0' && c <= '9') {
                ...
                if (productValue > MAX_SEC_KEY_VALUE) {
                    throw new HandshakeException("Key value too large: " + productValue);
                }
            } else if (c == ' ') { // count spaces
                spacesNum++;
                if (spacesNum > 12) {
                    throw new HandshakeException("Too many spaces: " + spacesNum);
                }
            } else if (c >= 0x21 && c <= 0x2F || c >= 0x3A && c <= 0x7E) {  // count chars
                charsNum++;
                if (charsNum > 12) {
                    throw new HandshakeException("Too many characters: " + charsNum);
                }
            } else {
                throw new HandshakeException("unexpected character: '" + c + "'");
            }
        }

        if (spacesNum < 1) {
            throw new HandshakeException("number of spaces should be more than 1");
        }

        if (charsNum < 1) {
            throw new HandshakeException("number of chars should be more than 1");
        }

        if (productValue % spacesNum != 0) {  // check if remainder is 0
            throw new HandshakeException("remainder is not 0");
        }
