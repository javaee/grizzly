...
public class Http2SessionOutputSink {
    ...
    protected <K> void writeDownStream(final K anyMessage,
        ...
        http2Session.getHttp2SessionChain().write(http2Session.getConnection(),
                null, msg, completionHandler, lifeCycleHandler);
    }
    ...
    protected void notifyWhenOperable(final WriteHandler writeHandler) {
    ...
    protected void writeDataDownStream(final Http2Stream stream,
                                       ...
                                       WritableMessage data,
                                       final CompletionHandler<WriteResult> completionHandler,
                                       final LifeCycleHandler messageCloner,
        ...
        if (data == null ||
            ...
            final DataFrame dataFrame = DataFrame.builder()
                    ...
                    .data((Buffer) data).endStream(isLast)
                    .build();
        ...
        if (messageCloner != null) {
            data = messageCloner.onThreadContextSwitch(http2Session.getConnection(), data);
        }
    ...
    private static class OutputQueueRecord extends AsyncQueueRecord<WriteResult> {
        ...
        public OutputQueueRecord(final int streamId,
            ...
            this.isZeroSizeData = !writableMessage.hasRemaining();
        ...
        private boolean isFinished() {
            return writableMessage == null;
        }

        private int serializeTo(final List<Http2Frame> frames,
                                final int maxDataSize) {

            final int recordSize = writableMessage.remaining();

            if (recordSize <= maxDataSize) {
                final DataFrame dataFrame = DataFrame.builder()
                        .streamId(streamId)
                        .data((Buffer) writableMessage).endStream(isLast)
                        .build();
                ...
                final Buffer b = (Buffer) writableMessage;
                final Buffer remainder = b.split(b.position() + maxDataSize);

                final DataFrame dataFrame = DataFrame.builder()
                        .streamId(streamId)
                        .data(b).endStream(false)
                        .build();

                frames.add(dataFrame);

                writableMessage = remainder;
