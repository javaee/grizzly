...
public class Http2AddOn implements AddOn {
    ...
    public void setup(NetworkListener networkListener, FilterChain filterChain) {
        ...
        final Http2ServerFilter http2Filter = updateFilterChain(filterChain);
        
        if (networkListener.isSecure()) {
            configureAlpn(transport, http2Filter, filterChain);
        }
    }
    ...
    private Http2ServerFilter updateFilterChain(final FilterChain filterChain) {
        
        final FilterReg httpServerFilter = filterChain.getRegByType(org.glassfish.grizzly.http.HttpServerFilter.class);
        ...
        filterChain.addAfter(httpServerFilter.filter(), http2HandlerFilter);
    ...
    private static void configureAlpn(final Transport transport,
        ...
        final FilterReg sslBaseFilter = filterChain.getRegByType(SSLBaseFilter.class);
        if (sslBaseFilter != null) {
            AlpnSupport.getInstance().configure((SSLBaseFilter) sslBaseFilter.filter());
