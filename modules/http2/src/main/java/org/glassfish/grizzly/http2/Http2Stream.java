...
public class Http2Stream implements AttributeStorage, OutputSink, Closeable {
    ...
    public GrizzlyFuture<Closeable> terminate() {
        ...
        close0(Futures.toCompletionHandler(future), CloseReason.LOCALLY_CLOSED_REASON, null, false);
    ...
    @Override
    public void terminateWithReason(final CloseReason reason) {
        close0(null, CloseReason.LOCALLY_CLOSED_REASON, null, false);
    }
    ...
    public GrizzlyFuture<Closeable> close() {
        final FutureImpl<Closeable> future = Futures.createSafeFuture();
        close0(Futures.toCompletionHandler(future), CloseReason.LOCALLY_CLOSED_REASON, null, true);
    ...
    void close0(
        ...
        if (closeReasonUpdater.compareAndSet(this, null, closeReason)) {
            
            final Termination termination = closeReason == CloseReason.LOCALLY_CLOSED_REASON ?
    ...
    public void addCloseListener(final CloseListener closeListener) {
        ...
        if (cr == null) {
            ...
            if (cr != null && closeListeners.remove(closeListener)) {
                ...
                try {
                    closeListener.onClosed(this, cr);
                ...
                if (closeFuture == null) {
                    ...
                    if (cr == null) {
                        ...
                        addCloseListener(new org.glassfish.grizzly.CloseListener() {
                            ...
                            @Override
                            public void onClosed(Closeable closeable, CloseReason closeReason)
                                    throws IOException {
                                final CloseReason cr = closeReason;
                                assert cr != null;
                                f.result(cr);
