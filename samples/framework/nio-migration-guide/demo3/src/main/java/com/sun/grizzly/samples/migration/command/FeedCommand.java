...
import com.sun.grizzly.samples.migration.response.handler.FeedResponseHandler;
import com.sun.grizzly.samples.migration.response.handler.IResponseHandler;

/**
 * The feed Command must be in the format
 * 
 * feed|symbol[eoq]
 * 
 ...
 * 
 * The connection with the client won't be close like
 * the quote command.
 * 
 * @author Sebastien Dionne
 *
 */
public class FeedCommand extends QuoteCommand {
	
	public FeedCommand(QuoteManager manager) {
		super(manager);
	}
	...
	public ICommandRequest process(String query, ClientConnectionHandler clientConnectionHandler){
		...
		f_responseHandler = new FeedResponseHandler();
		f_responseHandler.setClientConnectionHandler(clientConnectionHandler);
		f_responseHandler.setCommand(this);
		
		
		// add the quoteSubscription to the Cache for this client
		f_manager.addQuoteSubcriptionForClientConnectionHandler(f_quoteSubscription, clientConnectionHandler);
		
		// look if the quoteSubscription is not in the cache
		IResponseHandler handler = f_manager.getResponseHandlerFromQuoteSubcription(f_quoteSubscription);
		if(handler!=null){
			System.out.println("QUOTE FEED FOUND IN CACHE = " + f_quoteSubscription);
		...
		try {
			f_manager.addQuoteSubcription(f_quoteSubscription, f_responseHandler);
			return connection.sendFeedRequest(f_quoteSubscription, f_responseHandler);
		} catch (Exception e) {
