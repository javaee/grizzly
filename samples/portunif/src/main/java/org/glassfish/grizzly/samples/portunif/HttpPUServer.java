...
public class HttpPUServer {
    ...
    public static class PortUnificationAddOn implements AddOn {
        ...
        public void setup(final NetworkListener networkListener,
            ...
            // are not recognized - assume it's HTTP.
            ...
            // now find the place to insert PUFilter in the HTTP FilterChain.
            // we'll insert the PUFilter right next to the TransportFilter
            final FilterReg transportFilterReg =
                    filterChain.getRegByType(TransportFilter.class);
            
            assert transportFilterReg != null;
            
            filterChain.addAfter(transportFilterReg.name(), puFilter);
        }
    }
}
