...
public class SNIAddOn implements AddOn {
    ...
    public void setup(NetworkListener networkListener, FilterChain filterChain) {
        final FilterReg sslFilterReg = filterChain.getRegByType(SSLBaseFilter.class);
        if (sslFilterReg != null) {
            // replace SSLBaseFilter with SNIFilter
            final SSLBaseFilter sslFilter =
                    (SSLBaseFilter) sslFilterReg.filter();
            ...
            filterChain.replace(sslFilterReg.name(), sniFilterLocal);
        }
    }
}
