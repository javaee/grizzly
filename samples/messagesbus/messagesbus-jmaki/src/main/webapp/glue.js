...
jmaki.sendMessage = function(message) {
     ...
     var body ="publish=jMakiMessagesBus&message=<script id='comet_1'>window.parent.jmaki.CometClient.callback({message : \""  + message + "\"});</script>";
     xhr.send(body);
}
...
jmaki.subscribe("/jmaki/runtime/loadComplete", function() {
     var header = "<html><head><title>jMaki Grizzly Comet Words Sample</title></head>";
     var c = new jmaki.CometClient("words?subscribe=jMakiMessagesBus&cometTechnique=http-streaming&message=" + header);    
});
